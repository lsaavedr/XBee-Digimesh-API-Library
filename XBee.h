...
/**
Represents a Series 1 Digimesh RX packet
*/
...
public:
	 ...
	 */
	 //DM XBeeRequest
	XBeeRequest(uint8_t apiId, uint8_t frameId, uint8_t radius);
	...
	uint8_t getApiId();
	/**
	Returns broadcast radius
	*/
	uint8_t getRadius();
	/**
	Changes broadcast radius. Range = 0 - 0x20, 0 for max hops
	*/
...
public:
	...
	//DM Payload Request with radius
	PayloadRequest(uint8_t apiId, uint8_t frameId, uint8_t *payload, uint8_t payloadLength, uint8_t radius);
	/**
...
public:
	/* 
	Creates tx request with choice of optioon and not the default frameID
	*/
	DMTxRequest(XBeeAddress64 &addr64, uint8_t option, uint8_t *payload, uint8_t payloadLength, uint8_t frameId, uint8_t radius);
	/* 
