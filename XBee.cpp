...
void XBeeResponse::getDMRxResponse(XBeeResponse &dmRxResponse) {
	...
	dmRx->getRemoteAddress64().setMsb((uint32_t(getFrameData()[1]) << 24) + (uint32_t(getFrameData()[2]) << 16) + (uint16_t(getFrameData()[3]) << 8) + getFrameData()[4]);
	dmRx->getRemoteAddress64().setLsb((uint32_t(getFrameData()[5]) << 24) + (uint32_t(getFrameData()[6]) << 16) + (uint16_t(getFrameData()[7]) << 8) + getFrameData()[8]);
}
...
XBeeRequest::XBeeRequest(uint8_t apiId, uint8_t frameId, uint8_t radius) {
	_apiId = apiId;
	_frameId = frameId;
	_radius = radius;
	}
...
uint8_t XBeeRequest::getRadius() {
	return _radius;
}

void XBeeRequest::setRadius(uint8_t radius) {
	_radius = radius;
}
...
//DM payload request
PayloadRequest::PayloadRequest(uint8_t apiId, uint8_t frameId, uint8_t *payload, uint8_t payloadLength, uint8_t radius) : XBeeRequest(apiId, frameId, radius) {
	_payloadPtr = payload;
	_payloadLength = payloadLength;
}
...
DMTxRequest::DMTxRequest(XBeeAddress64 &addr64, uint8_t option, uint8_t *data, uint8_t dataLength, uint8_t frameId, uint8_t radius) : PayloadRequest(TX_REQUEST, frameId, data, dataLength, radius)
{
	_addr64 = addr64;
	_option = option;
	_radius = radius;
...
DMTxRequest::DMTxRequest(XBeeAddress64 &addr64, uint8_t *data, uint8_t dataLength) : PayloadRequest(TX_REQUEST, DEFAULT_FRAME_ID, data, dataLength, RADIUS)
{
	_addr64 = addr64;
	_option = ACK_OPTION;
	_radius = RADIUS;
